...
void ofApp::draw(){
    ...
    if (simpleHands.size() > 0) {
        if (simpleHands[0].fingers.size() > 0) {
            ...
            ofVec3f handDir = simpleHands[0].handPos - simpleHands[0].handNormal;
            ...
            m.addIntArg(-fingerDir.x / 2);
            m.addIntArg(-fingerDir.y / 2);
            m.addIntArg(fingerDir.z);
            
            
            cout << handDir.x << " " << handDir.y << " " << handDir.z << endl;
            
            /*
             m.addIntArg(handDir.x);
             m.addIntArg(handDir.y);
             m.addIntArg(handDir.z);
             */
